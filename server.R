...
function(input, output, session) {
  ...
  output$plot1 <- renderPlotly({
    ...
    annotations <- tibble(
      ...
      pred_price = price_predictor(fit(), given_details()),
    ...
    df %>%
      ...
      geom_point(data = annotations, aes(x = pred_year, y = pred_price, size = pred_hours), color = "Red", shape = 4) +
      geom_point(data = annotations, aes(x = pred_year, y = comp_price, size = pred_hours), color = "Blue", shape = 4) +
      geom_point(data = annotations, aes(x = pred_year, y = average_price, size = pred_hours), color = "Green", shape = 4)
  })
  ...
  output$plot2 <- renderPlotly({
    ...
    df %>%
      ...
      geom_point() +
      geom_smooth(method = "lm") +
      geom_point(data = annotations, aes(x = pred_hours, y = pred_price, size = pred_year), color = "Red", shape = 4) +
      geom_point(data = annotations, aes(x = pred_hours, y = comp_price, size = pred_year), color = "Blue", shape = 4) +
      geom_point(data = annotations, aes(x = pred_hours, y = average_price, size = pred_year), color = "Green", shape = 4)
  })
  ...
  output$plot3 <- renderPlotly({
    ...
    df %>%
      ...
      geom_point() +
      geom_smooth(method = "lm") +
      geom_point(data = annotations, aes(x = pred_condition_index, y = pred_price, size = pred_year), color = "Red", shape = 4) +
      geom_point(data = annotations, aes(x = pred_condition_index, y = comp_price, size = pred_year), color = "Blue", shape = 4) +
      geom_point(data = annotations, aes(x = pred_condition_index, y = average_price, size = pred_year), color = "Green", shape = 4)
  })
  ...
  output$dt_summary <- renderDT({
    ...
    if (!is.null(master_list)) {
      ...
      for (i in unit_list) {
        ...
        i_data <- tibble(
          year = master_list$Equip_List[[i]]$year,
